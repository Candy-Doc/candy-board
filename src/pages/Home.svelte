<script lang="ts">
  ...
  onMount(async () => {
    const mdGettingStarted = await GetReadmeContent(
      window.location.pathname,
      "src/markdown/GETTING_STARTED.md"
    );
  ...
  const getIds = (content: string | undefined) => {
    ...
    inlineParse[0].children?.forEach((token, index) => {
      if (
        ...
        index < inlineParse[0].children.length - 1
      ) {
        storeHtmlInlineAndInnerHtml.push({
          tokenContent: token.content,
          innerText: inlineParse[0].children[index + 1].content,
        });
      }
    });
